...
#only de function check was modified in this file. 
...
#TODO Use other means to override properly this function with out coping the modules :  instance, workitem. 
#In fact, wkf_expr don't contains class to use standard inheritance. 
...
def check(cr, workitem, ident, transition, signal):
    ...
    if transition['group_id'] and uid != 1:
        ...
        if not transition['group_id'] in user_groups:
            return False
    test = _eval_expr(cr, ident, workitem, transition['condition'])
    # if test is false then display message from transition 
    if not test and transition['display_msg']:
        raise osv.except_osv(_('Warning!'), transition['notcondition_msg'] )
